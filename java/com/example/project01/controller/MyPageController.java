...
public class MyPageController {
    ...
    // 비밀번호 변경 (마이페이지)
    @GetMapping("/setpass")
    public String setPassForm(@SessionAttribute(name = "loginMember", required = false) MemberDTO loginMember, Model model){
        // 로그인 상태 아니라면 메인으로 리다이렉트
        if (loginMember == null)
            return "redirect:/";
    ...
    @PostMapping("/setpass")
    public String setPass(@SessionAttribute(name = "loginMember", required = false) MemberDTO loginMember, Model model,
        ...
        boolean result = memberService.updatePass(new MemberDTO(loginMember.getEmail(), member.getNewPass()));
        try {
            // 비밀번호 유효성 검사
    ...
    @GetMapping("/mypost")
    public String seeMyPost(Criteria criteria, Model model, @SessionAttribute(name = "loginMember", required = false) MemberDTO loginMember){
        // 로그인 상태 아니라면 메인으로 리다이렉트
        if (loginMember == null)
            return "redirect:/";
    ...
    public String seeMyComment(Criteria criteria, Model model, @SessionAttribute(name = "loginMember", required = false) MemberDTO loginMember){
        // 로그인 상태 아니라면 메인으로 리다이렉트
        if (loginMember == null)
            return "redirect:/";
        ...
        criteria.setWriter(loginMember.getNickname());
        List<BoardCommentDTO> result = myPageService.getMyComment(criteria);
