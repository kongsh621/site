...
public class InfoBoardController {
    ...
    @GetMapping("/")
    public String index(@ModelAttribute InfoBoardDTO infoBoardDTO, Model model, Criteria criteria,
                        @SessionAttribute(name = "loginMember", required = false) MemberDTO loginMember,
                        @SessionAttribute(name = "favorites", required = false)Set<JjimDTO> favorites,
        ...
        // 일반 정렬
        List<InfoBoardDTO> result = infoBoardService.getListWithPaging(criteria1);
    ...
    // 버튼 정렬
    @RequestMapping("/sort")
    public String  sortByType(@SessionAttribute(name = "loginMember", required = false) MemberDTO loginMember,
        ...
        List<InfoBoardDTO> sorted = infoBoardService.sortByType(map);
    ...
    public String writeInfo(@ModelAttribute InfoBoardDTO infoBoard,
                            @SessionAttribute(name = "loginMember", required = false) MemberDTO loginMember,
    ...
    public String writeInfoForm(@SessionAttribute(name = "loginMember", required = false) MemberDTO loginMember,
    ...
    @GetMapping("/readinfo")
    public String readInfo(@SessionAttribute(name = "loginMember", required = false) MemberDTO loginMember,
                           Model model, @ModelAttribute InfoBoardDTO infoBoardDTO,
                           Criteria criteria){
        model.addAttribute("loginSession", loginMember);
        InfoBoardDTO result = infoBoardService.readById(infoBoardDTO.getId());
    ...
    @PostMapping("/rate")
    public String leaveRating(@ModelAttribute InfoBoardDTO infoBoardDTO, Model model, RedirectAttributes ra,
                              Criteria criteria, @SessionAttribute(name = "loginMember", required = false) MemberDTO loginMember) {
        model.addAttribute("loginSession", loginMember);

        try {
            Long id = infoBoardDTO.getId();
            // 평점 남길 수 있는지 확인
            ...
            ratingsDTO.setMember_id(loginMember.getId());

            // 이미 작품 평점 기록이 존재하면 리턴시킨다.
            if (infoBoardService.isExistRating(ratingsDTO)){
                return redirect.sendRedirect(ra, id, criteria, "평점은 한 번만 남길 수 있습니다.", "redirect:/infoboard/readinfo");
            }

            boolean insertResult = infoBoardService.insertRating(ratingsDTO);
            if (!insertResult){
            ...
            infoBoardDTO.setTotal_rate(getAvgByBoard.getRating());
            infoBoardDTO.setTotal(getAvgByBoard.getCount());
            boolean updateTotalRateResult = infoBoardService.updateTotalRate(infoBoardDTO);
    ...
    public String updateInfo(@SessionAttribute(name = "loginMember", required = false) MemberDTO loginMember,
                             Model model, RedirectAttributes ra, @ModelAttribute(name = "resultDTO") ResultDTO resultDTO,
                             @ModelAttribute InfoBoardDTO infoBoardDTO, Criteria criteria){
        log.info("update info = {}", infoBoardDTO);
        model.addAttribute("loginSession", loginMember);

        boolean result = false;
        try {
            result = infoBoardService.update(infoBoardDTO);
            if (result) {
                ra.addAttribute("id", infoBoardDTO.getId());
            ...
            if (resultDTO.isSuccess()){
                ...
                ra.addFlashAttribute("resultDTO", new ResultDTO(false, "update", infoBoardDTO));
    ...
    public String updateInfoForm(@SessionAttribute(name = "loginMember", required = false) MemberDTO loginMember,
                                 Model model, RedirectAttributes ra, @ModelAttribute(name = "resultDTO") ResultDTO resultDTO,
                                 @ModelAttribute InfoBoardDTO infoBoardDTO, Criteria criteria){
        model.addAttribute("loginSession", loginMember);
        // 받은 id로 데이터 읽어와 보낸다
        InfoBoardDTO inforboard = infoBoardService.readById(infoBoardDTO.getId());
        ...
        System.out.println("resultDTO = " + resultDTO + "infoboard" + infoBoardDTO);
        return "siteInfoWrite";
    }

    @GetMapping("/deleteinfo")
    public String deleteInfo(@SessionAttribute(name = "loginMember", required = false) MemberDTO loginMember,
    ...
    @GetMapping("/like")
    public String change_is_like(@ModelAttribute JjimDTO jjimDTO, RedirectAttributes ra, Model model,
                                 @SessionAttribute(name = "loginMember", required = false) MemberDTO loginMember,
        ...
        try {
            // 찜 테이블에 존재하는지 확인
            boolean isExitst = infoBoardService.isExistJjim(jjimDTO);
            System.out.println("Is this <borad-member> record Exists in Jjim table? = " + isExitst);
            if (isExitst){
                // 찜 유무 변경/ 찜으로 등록
                if (jjimDTO.getIs_like().equals("Y")){
                    // 존재한다면 상태만 바꿔줌
                    boolean result = infoBoardService.changeIsLike(jjimDTO);
                    System.out.println("Updated Jjim = " + result);
                    if (result){
                        Set<JjimDTO> list = infoBoardService.getJjimList(); // 상태 바꾼 거 가져온다
                ...
                JjimDTO result = infoBoardService.insertJjim(jjimDTO);
    ...
    public String favoritesList(@SessionAttribute(name = "loginMember", required = false) MemberDTO loginMember, Model model,
        ...
        if (loginMember != null){
            ...
            Set<InfoBoardDTO> result = infoBoardService.getJjimListWithPaging(map);
